  ...
  try {
    ...
    if (completion.choices && completion.choices.length > 0) {
      ...
      if (failures.length === 0) {
        res.status(200).json({ message: 'All lessons generated successfully', num_users: users.length, status_code: 200});
        return;
        ...
        res.status(200).json({ message: 'Some lessons generated successfully', status_code: 200, num_users: users.length, failures });
        return;
      }
    } else {
